...
vec3 calculateAmbientContribution(vec3 normal, vec3 view_dir, float ndotv, vec3 F0, float ao, PBRParameter pbr_params) {
...
vec3 calculateLightIntensity(in Light light, in vec3 view_dir, in vec3 normal, in PBRParameter pbr_params) {
   ...
   return Lo + calculateAmbientContribution(normal, view_dir, ndotv, F0, ao, pbr_params) + pbr_params.emissive;
}
#endif